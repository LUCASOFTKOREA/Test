...
<title>Kospi 스팀스탁지수 TOP50</title>
...
try {
 ...
 //////////////////////////////////////
 //원본 String query = "SELECT STOCK_NAME,STOCK_CODE,IDX_NOW,5MIN_READ_CNT,format(IDX_NOW*5MIN_READ_CNT,0) AS ST_IDX, STOCK_PRICE_TODAY, STOCK_PRICE_RATE_TODAY FROM STEAMSTOCK.STOCK_IDX ORDER BY cast(5MIN_READ_CNT as unsigned)*IDX_NOW desc, cast(idx_now as unsigned) desc limit 50";
 ...
 if("day".equals(range)){
	 query = "SELECT STIDX.STOCK_NAME,STIDX.STOCK_CODE,STIDX.IDX_NOW,STIDX.5MIN_READ_CNT,FORMAT(HIS.ST_IDX,0) AS ST_IDX, HIS.ST_IDX_SUM, STIDX.STOCK_PRICE_TODAY, STIDX.STOCK_PRICE_RATE_TODAY FROM (SELECT STOCK_CODE, cast(REPLACE(format(AVG(cast(5MIN_IDX as unsigned)*cast(5MIN_READ_CNT as unsigned)),0),',','') as unsigned) AS ST_IDX, cast(REPLACE(format(SUM(cast(5MIN_IDX as unsigned)*cast(5MIN_READ_CNT as unsigned)),0),',','') as unsigned) AS ST_IDX_SUM FROM STEAMSTOCK.STOCK_IDX_5MIN_HISTORY WHERE DATE=DATE_FORMAT(DATE_ADD(NOW(), INTERVAL 9 HOUR),'%Y-%m-%d') group by STOCK_CODE ORDER BY ST_IDX DESC ) HIS, STEAMSTOCK.STOCK_IDX STIDX WHERE HIS.STOCK_CODE=STIDX.STOCK_CODE ORDER BY HIS.ST_IDX DESC, HIS.ST_IDX_SUM DESC limit 50";	 
 }else if("daysum".equals(range)){
	 query = "SELECT STIDX.STOCK_NAME,STIDX.STOCK_CODE,STIDX.IDX_NOW,STIDX.5MIN_READ_CNT,FORMAT(HIS.ST_IDX,0) AS ST_IDX, HIS.ST_IDX_AVG, STIDX.STOCK_PRICE_TODAY, STIDX.STOCK_PRICE_RATE_TODAY FROM (SELECT STOCK_CODE, cast(REPLACE(format(AVG(cast(5MIN_IDX as unsigned)*cast(5MIN_READ_CNT as unsigned)),0),',','') as unsigned) AS ST_IDX_AVG, cast(REPLACE(format(SUM(cast(5MIN_IDX as unsigned)*cast(5MIN_READ_CNT as unsigned)),0),',','') as unsigned) AS ST_IDX FROM STEAMSTOCK.STOCK_IDX_5MIN_HISTORY WHERE DATE=DATE_FORMAT(DATE_ADD(NOW(), INTERVAL 9 HOUR),'%Y-%m-%d') group by STOCK_CODE ORDER BY ST_IDX DESC ) HIS, STEAMSTOCK.STOCK_IDX STIDX WHERE HIS.STOCK_CODE=STIDX.STOCK_CODE ORDER BY HIS.ST_IDX DESC limit 50";
 }else{
	 query = "SELECT STOCK_NAME,STOCK_CODE,IDX_NOW,5MIN_READ_CNT,format(IDX_NOW*5MIN_READ_CNT,0) AS ST_IDX, STOCK_PRICE_TODAY, STOCK_PRICE_RATE_TODAY FROM STEAMSTOCK.STOCK_IDX ORDER BY cast(5MIN_READ_CNT as unsigned)*IDX_NOW desc, cast(idx_now as unsigned) desc limit 50"; 
 }
...
<!-- STRONG>Kospi통계 - 스팀스탁지수 TOP50<br><br>(5분간격 실시간 데이터)<br><br> <%=search_date%></STRONG-->
<STRONG>Kospi통계 - 스팀스탁지수 TOP50<br><br>(5분간격 <%=rangeLabel%> 데이터)<br><br> <%=search_date%><br><br>[<a href="/idx_top50.jsp?range=lately">실시간</a>,<a href="/idx_top50.jsp?range=day">일평균</a>,<a href="/idx_top50.jsp?range=daysum">일일합계</a>]</STRONG>
</td>
</tr>
<tr>
<td colspan="2">
...
<STRONG>Kospi통계 - 스팀스탁지수 TOP50<br><br>(5분간격 <%=rangeLabel%> 데이터)<br><br> <%=search_date%> [<a href="/idx_top50.jsp?range=lately">실시간</a>,<a href="/idx_top50.jsp?range=day">일평균</a>,<a href="/idx_top50.jsp?range=daysum">일일합계</a>]</STRONG>
</td>
</tr>
<tr>
<td colspan="9">
...
<td align="center"> 
<a href="/idx_5min_history.jsp?stock_code=<%= rs.getString("STOCK_CODE")%>&stock_name=<%=rs.getString("STOCK_NAME")%>">표보기</a> | 
<a href="/idx_5min_history_chart.jsp?stock_code=<%= rs.getString("STOCK_CODE")%>&stock_name=<%=rs.getString("STOCK_NAME")%>">챠트보기</a>
</td>
<!--td align="center"> <a href="/board.jsp?stock_code=<%= rs.getString("STOCK_CODE")%>&stock_name=<%=rs.getString("STOCK_NAME")%>">최신글보기</a> </td>
<td align="center"> <a href="/board_top.jsp?stock_code=<%= rs.getString("STOCK_CODE")%>&stock_name=<%=rs.getString("STOCK_NAME")%>">인기글보기</a> </td-->
</tr>
<%	
}
%>
